var QuestionListModule = (function () {
  ...
  function eventBind() {
    ...
    $('body').on('click','.js-del_btn',clickDeliteButtons)
  ...
  function getQuestionList() {
    ...
    $.ajax(settings).done(function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        listHml += '<div class="question_cnt"><table class="question_table js-showQuestionPop" data-type="0" data-id="' + response[i].id + '">'
  ...
  function showQuestionPop(e) {
    if (!$(e.target).hasClass('js-del')) {
      var this_type = this.getAttribute('data-type') // 0:question detail, 1: question delete
      if (this_type == 0) {
      ...
      showPopupEvent(this_type)
    }
  }

  function showPopupEvent(this_type) {
    var el = document.querySelector('.pop_wrap')
    document.querySelector('.pop_bg').style.display = 'block';
    el.style.display = "block"
    el.style.visibility = "hidden"
    var content_el = document.querySelectorAll('.js-pop_content');
    for (var i = 0; i < content_el.length; i++) {
      if (i == this_type) {
        content_el[i].style.display = 'block'
      } else {
        content_el[i].style.display = 'none'
      }
    }
  ...
  function clickDeliteButtons(){
    ...
    console.log('test2')
      var settings = {
        "async": true,
        "url": "/api/v1/sam/session_talk/" + delete_question_id + "/delete_session_talk",
        "method": "POST",
        "headers": {
          "accept": "application/json"
        }
      ...
      $.ajax(settings).done(function (response) {
        ...
        if (response.status == 411) {
          document.querySelector('.h1-pop_title').innerHTML = ''
          document.querySelector('.question_text').innerHTML = '해당 게시글을 찾을 수 없습니다.'
          document.querySelector('.question_date_text').innerHTML = ''
          showPopupEvent(0)
        }
      });
  }
